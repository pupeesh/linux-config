...
//#include "/home/pupesh/.cache/wal/colors-wal-dwm.h"
/* appearance */
...
static const char *togglelayoutcmd[] = { "/bin/sh", "-c", "current$(setxkbmap -query | grep layout | awk '{print $2}'); if [ \"$current\" = \"us\" ]; then setxkbmap bg phonetic' else setxkbmap us; fi", NULL };
...
static Key keys[] = {
    ...
    { MODKEY|ControlMask|ShiftMask, XK_s,                      spawn,          SHCMD("systemctl suspend")},
		{ ALT_SHIFT,										XK_Shift_L,							   toggle_layout,		{.v = togglelayoutcmd } },
};
